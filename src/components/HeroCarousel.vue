...
<template>
	<h1 class="scroll-m-20 px-16 py-4 text-2xl font-extrabold tracking-tight lg:text-2xl uppercase">New & Upcoming Movies In Theaters</h1>
	<Carousel
		class="carousel"
		:opts="{
			loop: true,
			...
			<CarouselItem class="relative max-h-[500px]" v-for="(item, index) in movies" :key="index">
				...
				<div class="absolute left-24 right-24 flex h-3/4 gap-4 sm:top-16 top-4">
					...
					<div class="flex flex-col gap-3">
						...
						<Button @click="openDetailView(item.id, item.media_type)" class="max-w-fit">Find out more</Button>
					</div>
				</div>
			</CarouselItem>
		</CarouselContent>
